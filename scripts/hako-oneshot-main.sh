...
if ! test -f ${github_lower_case}-oneshot-main.jsonnet ; then
    echo "${github_lower_case}-oneshot-main.jsonnet not found"
    exit 1;
fi
...
bundle exec hako oneshot ${github_lower_case}-oneshot-main.jsonnet -e DISABLE_DATABASE_ENVIRONMENT_CHECK=1 -- $@
